...
def _is_near_duplicate_title(left_title: str, right_title: str) -> bool:
    ...
    if left_tokens and right_tokens:
        ...
        # Catch syndicated paraphrases that keep the same lead phrase
        # but differ in a few verbs/adjectives.
        leading_match = _leading_token_match_len(left, right)
        if leading_match >= 3 and overlap_ratio >= 0.75 and jaccard >= 0.6:
            return True

    return SequenceMatcher(None, left, right).ratio() >= 0.93


def _leading_token_match_len(left: str, right: str) -> int:
