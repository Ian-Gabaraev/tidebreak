...
- Return meaningful error messages in result context
...
- Created modular package architecture with 6 core modules
...
- Added Vietnam country source mapping with 5 provided English-language sources
...
- Added scoring-based ranking to prefer article-like links over topic/tag/navigation links
...
- Added regression test for same story appearing from two different websites
...
- Added near-duplicate title matching using token overlap + sequence similarity heuristics
- Preserved deterministic behavior by retaining first-seen article in duplicate clusters
- Added regression test for minor wording differences between syndicated titles
...
- Kept dedupe + quality filtering while simplifying consumer-facing payload format
...
- Added English-language Thailand source set under `TH`
